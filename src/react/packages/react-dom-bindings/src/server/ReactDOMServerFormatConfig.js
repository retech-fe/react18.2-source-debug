...
import { enableFilterEmptyStringAttributesDOM, enableCustomElementPropertySupport, enableFloat } from 'shared/ReactFeatureFlags';
...
import { prepareToRenderResources, finishRenderingResources, resourcesFromLink, ReactDOMServerDispatcher } from './ReactDOMFloatServer';
...
export function writeInitialResources(destination, resources, responseState) {
  ...
  const {
    ...
    explicitPreloads,
    implicitPreloads
  } = resources; // Flush stylesheets first by earliest precedence
...
function writeStyleResourceDependencies(destination, boundaryResources) {
  ...
  boundaryResources.forEach(resource => {
    if (resource.inShell) {// We can elide this dependency because it was flushed in the shell and
