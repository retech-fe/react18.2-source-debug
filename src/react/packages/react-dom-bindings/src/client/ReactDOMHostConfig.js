...
import { enableCreateEventHandleAPI, enableScopeAPI, enableFloat } from 'shared/ReactFeatureFlags';
import { HostComponent, HostResource, HostText } from 'react-reconciler/src/ReactWorkTags';
...
import { prepareToRenderResources, cleanupAfterRenderResources, isHostResourceType } from './ReactDOMFloatClient';
  ...
  for (; node != null; node = node.nextSibling) {
    ...
    if (enableFloat) {
      if (nodeType === ELEMENT_NODE) {
        if (isHostResourceInstance(node)) {
          continue;
        }
...
function isHostResourceInstance(instance) {
  if (instance.nodeType === ELEMENT_NODE) {
    // $FlowFixMe[prop-missing] Flow doesn't understand `nodeType` test.
    switch (instance.tagName.toLowerCase()) {
      case 'link':
        {
          const rel = instance.rel;
          return rel === 'preload' || // $FlowFixMe[prop-missing] Flow doesn't understand `nodeType` test.
