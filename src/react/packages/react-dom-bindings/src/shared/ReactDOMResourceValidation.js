...
export function validateUnmatchedLinkResourceProps(pendingProps, currentProps) {
  if (__DEV__) {
    if (pendingProps.rel !== 'font' && pendingProps.rel !== 'style') {
      if (currentProps != null) {
        ...
        const pendingHrefStatement = typeof pendingProps.href === 'string' ? ` and the updated href is "${pendingProps.href}"` : '';
        console.error('A <link> previously rendered as a %s but was updated with a rel type that is not' + ' valid for a Resource type. Generally Resources are not expected to ever have updated' + ' props however in some limited circumstances it can be valid when changing the href.' + ' When React encounters props that invalidate the Resource it is the same as not rendering' + ' a Resource at all. valid rel types for Resources are "font" and "style". The previous' + ' rel for this instance was %s. The updated rel is %s%s.', originalResourceName, originalRelStatement, pendingRelStatement, pendingHrefStatement);
      } else {
        const pendingRelStatement = getValueDescriptorExpectingEnumForWarning(pendingProps.rel);
        console.error('A <link> is rendering as a Resource but has an invalid rel property. The rel encountered is %s.' + ' This is a bug in React.', pendingRelStatement);
      }
    }
  }
}
...
function warnDifferentProps(href, originalName, latestName, extraProps, missingProps, differentProps) {
  if (__DEV__) {
    const juxtaposedNameStatement = latestName === originalName ? 'an earlier instance of this Resource' : `a ${originalName} with the same href`;
...
export function validateHrefKeyedUpdatedProps(pendingProps, currentProps) {
  if (__DEV__) {
    ...
    if (pendingProps.href === currentProps.href) {
      ...
      if (missingProps || extraProps || differentProps) {
        const latestWarningName = getResourceNameForWarning('style', currentProps, false);
        let comparisonStatement = '';
        ...
        default:
          {
            ...
            console.error('ReactDOM.preload() expected a valid "as" type in the options (second) argument but found %s instead.' + ' Please use one of the following valid values instead: %s. The href for the preload call where this' + ' warning originated is "%s".', typeOfAs, '"style" and "font"', href);
          }
      }
    }
  }
    ...
    if (!href || typeof href !== 'string') {
      ...
      switch (as) {
        ...
        default:
          {
            const typeOfAs = getValueDescriptorExpectingEnumForWarning(as);
            console.error('ReactDOM.preinit() expected the second argument to be an options argument containing at least an "as" property' + ' specifying the Resource type. It found %s instead. Currently, the only valid resource type for preinit is "style".' + ' The href for the preinit call where this warning originated is "%s".', typeOfAs, href);
          }
      }
    }
  }
