  ...
  function withEnableLegacyFBSupport(enableLegacyFBSupport) {
    describe('enableLegacyFBSupport ' + (enableLegacyFBSupport ? 'enabled' : 'disabled'), () => {
      ...
      it('does not invoke an event on a parent tree when a subtree is dehydrated', async () => {
        ...
        jest.runAllTimers();
        Scheduler.unstable_flushAll(); // The Suspense boundary is not yet hydrated.
      ...
      it('should preserve bubble/capture order between roots and nested portals', () => {
        ...
        if (enableLegacyFBSupport) {
          ...
          expect(log).toEqual([// The events on root probably shouldn't fire if a non-React intermediated. but current behavior is that they do.
      ...
      describe('ReactDOM.createEventHandle', () => {
        ...
        it('should correctly work for a basic "click" listener', () => {
          ...
          ReactDOM.render(<Test />, container);
          Scheduler.unstable_flushAll();
          expect(container.innerHTML).toBe('<button><div>Click me!</div></button>'); // Clicking the button should trigger the event callback
          ...
          log = []; // Clicking the button should also work
        ...
        it('should work with concurrent mode updates', async () => {
          ...
          expect(Scheduler).toFlushAndYield(['Test']); // Click the button
          ...
          expect(Scheduler).toFlushAndYieldThrough(['Test']); // Click the button again
        ...
        it('should correctly work for a basic "click" window listener', () => {
          ...
          ReactDOM.render(<Test />, container);
          Scheduler.unstable_flushAll();
          expect(container.innerHTML).toBe('<button>Click anything!</button>'); // Clicking outside the button should trigger the event callback
        ...
        it('beforeblur and afterblur are called after a focused element is unmounted', () => {
          const log = []; // We have to persist here because we want to read relatedTarget later.
        ...
        describe('Compatibility with Scopes API', () => {
          ...
          it('should not handle the target being a dangling text node within a scope', () => {
            ...
            Scheduler.unstable_flushAll();
            const textNode = buttonRef.current.firstChild;
            dispatchClickEvent(textNode); // This should not work, as the target instance will be the
          ...
          it('should be able to register handlers for events affected by the intervention', () => {
            ...
            expect(allEvents).toEqual(['touchstart', 'touchmove', 'wheel']); // These events are passive by default, so we can't preventDefault.
