...
import { OffscreenVisible, OffscreenPassiveEffectsConnected } from './ReactFiberOffscreenComponent';
...
function commitUseEventMount(finishedWork) {
  ...
  if (eventPayloads !== null) {
    ...
    for (let ii = 0; ii < eventPayloads.length; ii += 2) {
      const eventFn = eventPayloads[ii];
...
function hideOrUnhideAllChildren(finishedWork, isHidden) {
  ...
  if (supportsMutation) {
    ...
    while (true) {
      if (node.tag === HostComponent || (enableFloat && supportsResources ? node.tag === HostResource : false)) {
...
function isHostParent(fiber) {
  return fiber.tag === HostComponent || fiber.tag === HostRoot || fiber.tag === HostPortal || (enableFloat && supportsResources ? fiber.tag === HostResource : false);
}
