...
import { ClassComponent, FunctionComponent, ForwardRef, HostComponent, HostResource, HostPortal, HostRoot, MemoComponent, SimpleMemoComponent } from './ReactWorkTags';
...
export function isCompatibleFamilyForHotReloading(fiber, element) {
  if (__DEV__) {
    ...
    if (needsCompareFamilies) {
      ...
      const prevFamily = resolveFamily(prevType); // $FlowFixMe[not-a-function] found when upgrading Flow
...
function findChildHostInstancesForFiberShallowly(fiber, hostInstances) {
  if (__DEV__) {
    ...
    while (true) {
      if (node.tag === HostComponent || (enableFloat ? node.tag === HostResource : false)) {
        // We got a match.
