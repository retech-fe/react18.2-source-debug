...
import { resetWakeableStateAfterEachAttempt, resetThenableStateOnCompletion, trackSuspendedWakeable, suspendedThenableDidResolve, isTrackingSuspendedThenable } from './ReactFiberWakeable.new';
...
function handleThrow(root, thrownValue) {
  ...
  ReactCurrentOwner.current = null; // Setting this to `true` tells the work loop to unwind the stack instead
...
function renderRootSync(root, lanes) {
  ...
  const prevDispatcher = pushDispatcher(root.containerInfo); // If the root or lanes have changed, throw out the existing stack
